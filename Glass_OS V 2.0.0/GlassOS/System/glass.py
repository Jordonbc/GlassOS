...
def starter():
    ...
    title = Label(root, text="Welcome to Glass OS V" + version, font=("arial", 32, "bold"))
    title.pack()
    ...
    def partTwo():  ## allows the user to change settings before starting up the program fully
        ...
        displayWidthTitle = Label(settingsFrame, text="Display Width:", font=("arial", 16))
        widthBox = Entry(settingsFrame, font=("arial", 16))
        widthBox.insert(index=0, string=screenWidth)

        displayHeightTitle = Label(settingsFrame, text="Display Height:", font=("arial", 16))
        heightBox = Entry(settingsFrame, font=("arial", 16))
        heightBox.insert(index=0, string=screenHeight)

        userSpaceTitle = Label(settingsFrame, text="User Directory:", font=("arial", 16))
        userSpaceBox = Entry(settingsFrame, font=("arial", 16))
        userSpaceBox.insert(index=0, string="Users/")

        backgroundTitle = Label(settingsFrame, text="Background:", font=("arial", 16))
        backgroundBox = Entry(settingsFrame, font=("arial", 16))
        backgroundBox.insert(index=0, string="Users/background.png")

        menuColourTitle = Label(settingsFrame, text="Menu Colour:", font=("arial", 16))
        menuColourChooser = Button(settingsFrame, text="Choose Colour", command=chooseColour, bg=menuColour,
                                   font=("arial", 16))
        menuColourBox = Entry(settingsFrame, font=("arial", 16))
        menuColourBox.insert(index=0, string="#a0fd44")
        colour = menuColour

        startPictureTitle = Label(settingsFrame, text="Start Menu Icon:", font=("arial", 16))
        startPictureBox = Entry(settingsFrame, font=("arial", 16))
        startPictureBox.insert(index=0, string="GlassOS/start.png")

        startFullTitle = Label(settingsFrame, text="Use fullScreen start:", font=("arial", 16))
        start_fullSlider = Scale(settingsFrame, from_=0, to=1, orient=HORIZONTAL, font=("arial", 16))
            ...
            def terminal(command=None):
                def run(command=None):
                    if input.get() == "cc":
                        try:
                            os.remove("GlassOS/Cache/terminalCache.py")
                            temp = open("GlassOS/Cache/terminalCache.py")
                            temp.close()
                            input.delete(0, END)
                            print("\nCache Cleared")
                        except:
    ...
    def check():
        ...
        try:
            ...
            if int(updateVer) > int(version):
                statusText.configure(text="Status: There is a new update available!")
